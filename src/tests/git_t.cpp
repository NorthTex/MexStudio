...
void Test::Git::basicFunctionality(){

    if(!useTests) // skip test in auto tests
        return;

    if(builder -> CMD_GIT.isEmpty())
        return;
...
bool Test::Git::runCommand(QString commandline,QString * buffer){
    ...
    return builder -> runCommand(commandline,QFileInfo(path),QFileInfo(),0,buffer,nullptr,buffer);
}
