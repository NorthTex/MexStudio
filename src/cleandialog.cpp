...
bool CleanDialog::checkClean(const LatexDocuments & documents){
	...
	scope -> blockSignals(true);  // don't emit currentIndexChanged while populating the combo box

	if(documents.masterDocument){

		masterFile = documents.masterDocument -> getFileName();
	...
	if(documents.currentDocument && documents.currentDocument -> getFileInfo().suffix().toLower() == "tex"){

		currentTexFile = documents.currentDocument->getFileName();
	...
	foreach(LatexDocument * document,documents.documents){
		if(document -> getFileInfo().suffix().toLower() == "tex")
