...
SpellerDialog::SpellerDialog(QWidget * parent,SpellerUtility * utility)
	...
	auto messageArea = new QLabel(m_statusBar);
	...
	auto itemDelegate = new IgnoreListViewDelegate(ui.ignoreListView);
...
void SpellerDialog::setEditorView(LatexEditorView * view){

    editor = view ? view -> editor : nullptr;
	editorView = view;

    if(view)
        mReplacementList = view -> getReplacementList();
}
...
void SpellerDialog::startSpelling(){
    ...
    auto dlh = editor -> document() -> line(curLine).handle();
...
void SpellerDialog::closeEvent(QCloseEvent * ce){
	...
	if(editor)
		editor -> setCursorPosition(startLine,startIndex);
...
void SpellerDialog::updateItem(){
	...
	int current = -1;
	auto items = ui.listSuggestions -> selectedItems();
...
void SpellerDialog::toggleIgnoreList(bool forceHide){
	...
	auto hideableWidgets = QList<QWidget *>() 
		<< ui.ignoreListView 
		<< ui.labelIgnoredWords 
		<< ui.pushButtonAdd 
		<< ui.pushButtonRemove 
		<< ui.labelAsHideableSpacer;

	if(ui.ignoreListView -> isVisible() || forceHide){

		for(auto widget : hideableWidgets)
			widget -> hide();
...
void SpellerDialog::addIgnoredWord(){
	...
	auto model = m_speller -> ignoreListModel();
	
	model -> insertRow(0);
	model -> setData(model -> index(0),QVariant(tr("<new>", "Placeholder for new added word in ignore list")));

	ui.ignoreListView -> selectionModel() -> setCurrentIndex(model -> index(0),QItemSelectionModel::Select);
	ui.ignoreListView -> edit(model -> index(0));
}
...
void SpellerDialog::removeIgnoredWord(){
	...
	if(!ui.ignoreListView -> model())
		return;
	
	QString selectedWord = ui.ignoreListView 
		-> model() 
		-> data(ui.ignoreListView -> currentIndex(),Qt::DisplayRole)
		.toString();
...
IgnoreListViewDelegate::IgnoreListViewDelegate(QObject * parent) 
	: QStyledItemDelegate(parent){

	auto creator = new QStandardItemEditorCreator<ValidatedLineEdit>();
