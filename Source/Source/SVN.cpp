...
SVN::Status SVN::status(QString filename){
	...
	const auto output = runSvn("status",quote(filename));

	if(output.isEmpty())
		return SVN::CheckedIn;
	
	if(output.startsWith('?'))
		return SVN::Unmanaged;

	if(output.startsWith('M'))
		return SVN::Modified;
	
	if(output.startsWith('C'))
		return SVN::InConflict;
...
QStringList SVN::log(QString filename){
    ...
    const auto output = runSvn("log",quote(filename));
...
void SVN::createRepository(QString filename){
	...
	const auto path = QFileInfo(filename).absolutePath();
