...
LatexReader::LatexReader(const QString & line)
    : lp(& LatexParser::getInstance()){
...
LatexReader::LatexReader(const LatexParser & parser,const QString & line)
    : lp(& parser){
...
LatexReader::LatexReader(const LatexParser & parser,const QString &line,QMap<QString, QString> & replacementList)
    : lp(& parser){
...
LatexReader::NextWordFlag LatexReader::nextWord(bool returnCommands){
	...
	while((wordStartIndex = nextToken(line, index, inEnv, !inReferenz)) != -1){
        ...
        if(word.length() == 0)
            return NW_NOTHING; //should never happen
		...
		case '\\':
			...
			if(
                word.length() == 1 || 
                !(ESCAPED_CHARS.contains(word.at(1)) || 
                CHARACTER_ALTERING_CHARS.contains(word.at(1)))
            ){
                ...
                if(
                    lp -> possibleCommands["%ref"].contains(word) || 
                    lp -> possibleCommands["%label"].contains(word) || 
                    lp -> possibleCommands["%cite"].contains(word) || 
                    lp -> possibleCommands["%usepackage"].contains(word)
                ){
				...
				if(
                    lp -> optionCommands.contains(lastCommand) || 
                    lastCommand.isEmpty() || 
                    word == "\\begin" || 
                    word == "\\end"
                ){
