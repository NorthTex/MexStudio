...
struct StructureEntry {
	...
	StructureEntry(LatexDocument *,Type); ///< constructor
	~StructureEntry();
	...
	void setLine(QDocumentLineHandle *,int lineNr = -1); ///< set linehandle for automatic update of line number
	...
	bool hasContext(Context context) const {
		return m_contexts & context;    ///< get context
	}

	void setContext(Context context,bool state = true){
		m_contexts = (state)
			? (m_contexts | context)
			: (m_contexts & ~context);
	}
...
class StructureEntryIterator {
	...
	public:
		...
		StructureEntryIterator(StructureEntry *);
