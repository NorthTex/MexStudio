...
struct ManagedProperty {
	...
	void valueFromQVariant(const QVariant);
...
#define CONFIG_DECLARE_OPTION_WITH_OBJECT(config, type, name, defaultvalue, configname, object)	   \
	static type name;																			   \
	config -> registerOption(configname,& name,defaultvalue);									   \
	config -> linkOptionToObject(& name,object,LinkOptions(LO_UPDATE_ALL | LO_DIRECT_OVERRIDE));
