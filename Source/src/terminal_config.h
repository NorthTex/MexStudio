		...
		void initDefaults(const QStringList& xf){
				...
				if(terminalFontFamily.isEmpty()){
					if(xf.contains("DejaVu Sans Mono",Qt::CaseInsensitive))
						terminalFontFamily = "DejaVu Sans Mono";
					else
					if(xf.contains("Lucida Sans Typewriter",Qt::CaseInsensitive))
						terminalFontFamily = "Lucida Sans Typewriter";
					else
						terminalFontFamily = qApp -> font().family();
				}

				if(terminalFontSize == -1)
					terminalFontSize = qApp -> font().pointSize();
			#endif
		}
